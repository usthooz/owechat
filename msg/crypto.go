...
func aesKeyDecode(encodedAESKey string) (key []byte, err error) {
	if len(encodedAESKey) != 43 {
		err = fmt.Errorf("aesKeyDecode: the length of encodedAESKey must be equal to 43")
		return
	}
	...
	if len(key) != 32 {
		err = fmt.Errorf("aesKeyDecode: encodingAESKey invalid")
		return
	}
	return
}
...
func AESDecryptMsg(ciphertext []byte, aesKey []byte) (random, rawXMLMsg, appID []byte, err error) {
	const (
		// PKCS#7
		BlockSize = 32
		// BLOCK_SIZE 为 2^n 时, 可以用 mask 获取针对 BLOCK_SIZE 的余数
...
// encodeNetworkByteOrder 把整数 n 格式化成 4 字节的网络字节序
...
// decodeNetworkByteOrder 从 4 字节的网络字节序里解析出整数
